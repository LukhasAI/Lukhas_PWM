    ...
    for module_name, class_name in class_checks:
        try:
            ...
            if hasattr(module, class_name):
                ...
                missing_components[f"{module_name}.{class_name}"] = f"Class {class_name} not defined in {module_name}"
        except ImportError as e:
            print(f"❌ {module_name}.{class_name}: MODULE MISSING - {str(e)}")
            missing_components[f"{module_name}.{class_name}"] = f"Module {module_name} not found"
